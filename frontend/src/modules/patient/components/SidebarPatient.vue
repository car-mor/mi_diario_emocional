      ...
      <nav class="flex flex-col space-y-4 md:space-y-6 flex-1 mt-12 md:mt-6 w-full px-2">
        ...
        <router-link
          ...
          :class="[
            ...
            // Clases condicionales para Móvil vs Desktop
            showMobileMenu ? 'justify-start px-4' : 'justify-center',
            'hover:bg-[#5aa7d1] dark:hover:bg-gray-700'
          ]"
          @click="showMobileMenu = false"
        >
          <component :is="item.icon" class="w-8 h-8 md:w-9 md:h-9 flex-shrink-0 transition-transform group-hover:scale-110" />
...
import { ref, markRaw } from "vue" // markRaw es buena práctica para componentes de iconos
import {
...
// Definimos la estructura de datos del menú
// Usamos markRaw para los iconos para optimizar el rendimiento de Vue
const menuItems = [
  { name: 'Mi Perfil', path: '/profile-patient-mobile', icon: markRaw(IconUserCircle), mobileOnly: true }, // Puedes filtrar este si quieres ocultarlo en desktop
